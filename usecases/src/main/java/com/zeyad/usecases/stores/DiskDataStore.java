...
public class DiskDataStore implements DataStore {
    ...
    public <M> Flowable<M> dynamicGetObject(
            ...
            boolean shouldCache) {
        String key =
                dataClass.getSimpleName() + (itemIdL == null ? itemIdS : String.valueOf(itemIdL));
        return withCache && Storo.contains(key)
                ? Utils.getInstance()
                        .toFlowable(
                                Storo.get(key, dataClass)
                                        .async()
                                        .map(object -> mEntityDataMapper.mapTo(object, dataClass)))
                : mDataBaseManager
                        .<M>getById(idColumnName, itemIdL, itemIdS, dataClass)
                        .doOnEach(
                                notification -> {
                                    try {
                                        if (withCache && !Storo.contains(key)) {
                                            cacheObject(
                                                    idColumnName,
                                                    new JSONObject(
                                                            gson.toJson(notification.getValue())),
                                                    dataClass);
                                        }
    ...
    public Flowable dynamicDeleteCollection(
        ...
        return mDataBaseManager
                .evictCollection(idColumnName, convertToListOfId, dataClass)
                .doOnEvent(
                        object -> {
                            if (withCache) {
                                for (int i = 0,
                                                        convertToListOfIdSize =
                                                                convertToListOfId != null
                                                                        ? convertToListOfId.size()
                                                                        : 0;
                                        i < convertToListOfIdSize;
                                        i++) {
                                    Storo.delete(
                                            dataClass.getSimpleName() + convertToListOfId.get(i));
                                }
                            }
                        })
                .toFlowable();
    ...
    private void cacheList(
            String idColumnName, @NonNull JSONArray jsonArray, @NonNull Class dataClass) {
    ...
    @NonNull
    @Override
    public <M> Flowable dynamicUploadFile(
            String url,
            File file,
            String key,
