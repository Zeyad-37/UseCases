...
public class RealmManager implements DataBaseManager {
    ...
    private static final String REALM_OBJECT_INVALID = "RealmObject is invalid",
            JSON_INVALID = "JSONObject is invalid",
            NO_ID = "Could not find id!";
    private static Handler backgroundHandler;

    public RealmManager(Looper backgroundLooper) {
        if (backgroundHandler == null) backgroundHandler = new Handler(backgroundLooper);
    }
                ...
                () -> {
                    if (itemIdL <= 0 && itemIdS == null) {
                        return Flowable.error(
                                new IllegalArgumentException("Id can not be less than Zero."));
                    }
                    Realm realm = Realm.getDefaultInstance();
                    return itemIdS == null
                            ? Utils.getInstance()
                                    .toFlowable(
                                            realm.where(dataClass)
                                                    .equalTo(idColumnName, itemIdL)
                                                    .findAll()
                                                    .asObservable()
                                                    .filter(
                                                            results ->
    ...
    @NonNull
    @Override
    public <M extends RealmModel> Flowable<List<M>> getQuery(
            @NonNull RealmQueryProvider<M> queryFactory) {
        return Flowable.defer(
                () -> {
                    Realm realm = Realm.getDefaultInstance();
                    return Utils.getInstance()
    ...
    @NonNull
    @Override
    public <M extends RealmModel> Completable put(
            @Nullable M realmModel, @NonNull Class dataClass) {
        if (realmModel != null) {
    ...
    public Completable put(
        ...
        } else return Completable.error(new IllegalArgumentException(JSON_INVALID));
    }
    ...
    public Completable putAll(
            @NonNull JSONArray jsonArray, String idColumnName, @NonNull Class dataClass) {
        return Completable.defer(
                () -> {
                    try {
                    ...
                    try {
                        executeWriteOperationInRealm(
                                realm, () -> realm.createOrUpdateAllFromJson(dataClass, jsonArray));
                        return Completable.complete();
                    } finally {
                        closeRealm(realm);
                    }
    ...
    public <T extends RealmModel> Completable putAll(List<T> realmObjects, Class dataClass) {
        return Completable.defer(
                () -> {
                    ...
                    try {
                        executeWriteOperationInRealm(
                                realm, () -> realm.copyToRealmOrUpdate(realmObjects));
                        return Completable.complete();
                    } finally {
                        closeRealm(realm);
                    }
     ...
     */
    @Override
    public boolean evictById(
            @NonNull Class clazz, @NonNull String idFieldName, final long idFieldValue) {
        Realm realm = Realm.getDefaultInstance();
        try {
    ...
    @NonNull
    @Override
    public Completable evictCollection(
            @NonNull String idFieldName, @NonNull List<Long> list, @NonNull Class dataClass) {
        return Completable.defer(
                () -> {
    ...
    private <T> T executeWriteOperationInRealm(
            @NonNull Realm realm, @NonNull ExecuteAndReturn<T> executor) {
        T toReturnValue;
        if (realm.isInTransaction()) realm.cancelTransaction();
        realm.beginTransaction();
    ...
    @NonNull
    private JSONArray updateJsonArrayWithIdValue(
            @NonNull JSONArray jsonArray, @Nullable String idColumnName, Class dataClass)
    ...
    @NonNull
    private JSONObject updateJsonObjectWithIdValue(
            @NonNull JSONObject jsonObject, @Nullable String idColumnName, Class dataClass)
            throws JSONException {
        if (idColumnName == null || idColumnName.isEmpty()) {
            throw new IllegalArgumentException(NO_ID);
        }
