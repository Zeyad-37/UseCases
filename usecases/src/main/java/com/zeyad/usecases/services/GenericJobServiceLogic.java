        ...
        if (extras.containsKey(GenericJobService.PAYLOAD)) {
            ...
            switch (extras.getString(GenericJobService.JOB_TYPE, "")) {
                case GenericJobService.POST:
                    Log.d(GenericJobServiceLogic.class.getSimpleName(), String.format(log, GenericJobService.POST));
                ...
                case GenericJobService.DOWNLOAD_FILE:
                    Log.d(GenericJobServiceLogic.class.getSimpleName(), String.format(log, GenericJobService.DOWNLOAD_FILE));
                    return new FileIO(trailCount, extras.getParcelable(GenericJobService.PAYLOAD),
                            null, true, cloudDataStore, utils).execute();
                case GenericJobService.UPLOAD_FILE:
                    Log.d(GenericJobServiceLogic.class.getSimpleName(), String.format(log, GenericJobService.UPLOAD_FILE));
                    return new FileIO(trailCount, extras.getParcelable(GenericJobService.PAYLOAD),
                            null, false, cloudDataStore, utils).execute();
                default:
